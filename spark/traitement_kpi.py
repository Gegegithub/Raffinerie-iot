...
from pyspark.sql.functions import col, from_json, window, avg, expr
...
# 6.1 Convertir le timestamp en type date/heure pour les données filtrées
filtrees = filtrees.withColumn("timestamp", expr("to_timestamp(timestamp)"))
...
kpi = filtrees.withColumn("ts", expr("to_timestamp(timestamp)")) \
    ...
    .withColumn("unite", expr(
        "CASE WHEN type_capteur = 'temperature' THEN '°C' ELSE 'mm/s' END"
    )) \
